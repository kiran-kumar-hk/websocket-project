...
class GitCSVEditor:
    def __init__(self, repo_url, branch_name, filename, token):
        ...
        self.repo = Repo.clone_from(self.authenticated_url(self.repo_url, self.token), self.repo_dir)
        self.repo.git.checkout(self.branch_name)
        self.filepath = os.path.join(self.repo_dir, self.filename)
        self.df = pd.read_csv(self.filepath)

    def authenticated_url(self, url, token):
...
def fetch_branches(repo_url, token):
    repo_dir = tempfile.mkdtemp()
    try:
        repo = Repo.clone_from(f"https://{token}@{repo_url.split('https://')[-1]}", repo_dir)
...
if repo_url and token:
    branches = fetch_branches(repo_url, token)
    ...
    if selected_branch:
        try:
            ...
            repo = Repo.clone_from(f"https://{token}@{repo_url.split('https://')[-1]}", temp_repo_dir)
        ...
        if selected_file:
            try:
                editor = GitCSVEditor(repo_url, selected_branch, selected_file, token)
